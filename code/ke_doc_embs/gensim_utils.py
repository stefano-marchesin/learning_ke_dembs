...
class Utils(object):
	...
	def read_corpus(self, corpus_folder):  # @smarchesin TODO: extend to CDS and PM collections too
	...
	def compute_idfs(self, corpus):
		...
		# produce Gensim's Dictionary from docs
		word_dict = Dictionary(corpus.values())
		# obtain document frequencies
		dfreqs = dict(word_dict.dfs)
		# return IDF scores
		return {word_dict[idx]: np.log(len(docs) / (1 + float(dfreq))) for idx, dfreq in dfreqs.items()}
	...
	def read_queries(self, query_path):  # @smarchesin TODO: extend to CDS and PM queries
	...
	def query2emb(self, query, model):
...
class EpochRanker(CallbackAny2Vec):
	...
	def on_epoch_end(self, model):
		...
		model.save(self.model_folder + '/' + self.opts.model_name + str(self.epoch + 1) + '.model')
		...
		for qid, qtext in self.queries.items():
			...
			q_emb = self.utils.query2emb(qtext[self.opts.qfield], model)
			if q_emb is None:
		...
		scores = self.utils.evaluate(['Rprec', 'P_5', 'P_10', 'P_20', 'ndcg', 'map'], self.rankings_folder, self.opts.model_name + '_' + str(self.epoch + 1), self.qrels_folder, self.opts.qrels_fname)
